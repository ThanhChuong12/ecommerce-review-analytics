...
------------------------
CLI script to train ResNet50, MobileNetV3, and EfficientNet-B0 on labeled image data.

Input data: labeled/train/ (ImageFolder format, after running split_image_dataset.py)
Output artifacts:
...
Run on CPU (~1.5–2h with default settings):
  python scripts/train_image_baseline.py --backbone mobilenet_v3

Run full training (Colab T4 GPU, ~40 mins):
...
# Add project root to sys.path to import ai_engine correctly
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train Transfer Learning model to identify damaged packaging"
    )
    parser.add_argument(
        "--backbone",
        choices=["resnet50", "mobilenet_v3", "efficientnet_b0", "all"],
        default="mobilenet_v3",
        help="Backbone to train. 'all' trains all 3 (default: mobilenet_v3)",
    )
    parser.add_argument(
        "--data-dir", default=DEFAULT_DATA_DIR,
        help="Training image directory (ImageFolder format). Default: labeled/train/",
    )
    parser.add_argument(
        "--val-dir", default=None,
        help="Physical validation image directory (ImageFolder format). If provided, fit() uses this folder and ignores val-split.",
    )
    parser.add_argument("--epochs", type=int, default=15,
        help="Maximum training epochs (default: 15 — early stopping usually triggers at epoch 8-12)")
    parser.add_argument("--batch-size", type=int, default=32, help="Batch size (default: 32)")
    parser.add_argument("--lr", type=float, default=1e-3, help="Learning rate (default: 0.001)")
    parser.add_argument(
        "--val-split", type=float, default=0.2,
        help="Internal validation split ratio. When using labeled/train/, set = 0.2 to let fit() split automatically. (default: 0.2)",
    )
    parser.add_argument("--patience", type=int, default=3, help="Early stopping patience (default: 3)")
    parser.add_argument("--weights-dir", default=DEFAULT_WEIGHTS_DIR, help="Directory to save .pt weights files")
    parser.add_argument(
        "--results-dir", default=DEFAULT_RESULTS_DIR,
        help="Directory to save plots, history, and error analysis (default: ai_engine/models/results)",
    )
    parser.add_argument(
        "--eval-only", action="store_true",
        help="Only evaluate the saved model on --data-dir without retraining",
    )
    parser.add_argument(
        "--eval-test", action="store_true",
        help="Evaluate the saved model on the physical Test set (labeled/test/). Used AFTER training to report official metrics.",
    )
    parser.add_argument(
        "--test-dir", default=DEFAULT_TEST_DIR,
        help="Test set directory (default: labeled/test/)",
    )
    parser.add_argument(
        "--subset-ratio", type=float, default=0.5,
        help="Ratio of training data to use (0 < x <= 1.0). Default: 0.5. Use 1.0 on GPU.",
    )
    parser.add_argument(
...
def train_single(
    ...
    """Train 1 backbone and save weights. Returns a metrics summary."""
    logger.info("--- Starting training: %s ---", backbone.upper())
    t_start = time.time()
    ...
    # Get evaluation results on val set (run at the end of training to avoid data leakage)
    logger.info("Retrieving evaluation results on validation set...")
    if hasattr(model, '_val_report') and model._val_report:
        eval_report = model._val_report
        logger.info("Using validation set results from training (no data leakage).")
    else:
        logger.info("Fallback: evaluating model %s on full dataset...", backbone)
    ...
    logger.info(
        "%s complete — Accuracy=%.4f | Macro-F1=%.4f | Defect F1=%.4f | Time=%.1f minutes",
...
def eval_only(
    ...
    """Load a trained model and evaluate it without retraining."""
    filename = weights_name if weights_name else f"{backbone}_defect.pt"
    weights_path = os.path.join(weights_dir, filename)
...
def main():
    ...
    # --- Evaluate on physical Test set ---
    if args.eval_test:
        if not os.path.exists(args.test_dir):
            logger.error(
                "Test set not found: %s\n"
                "Run scripts/split_image_dataset.py first to create labeled/test/",
                args.test_dir,
            )
            sys.exit(1)
        logger.info("=" * 60)
        logger.info("EVALUATE ON TEST SET: %s", args.test_dir)
        logger.info("(This test set is NOT used in any training steps)")
        logger.info("=" * 60)
        ...
        if args.threshold_file and os.path.exists(args.threshold_file):
            try:
                ...
                logger.error("Error reading threshold file: %s", e)
        ...
        logger.info("Test set results saved to: %s", test_results_path)
    ...
    # --- Train or evaluate on train/val set ---
    if not os.path.exists(args.data_dir):
        logger.error(
            "Data directory not found: %s\n"
            "Run scripts/split_image_dataset.py to create labeled/train/ first.",
            args.data_dir,
        )
        sys.exit(1)
